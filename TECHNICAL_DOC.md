...
| **RazorpayX** | Payouts / escrow release | ✅ | – | – | Fund transfers, balance check, payout-method registration | Raw REST `api.razorpay.com` (X endpoints) in `register-payout-method`, `razorpayx-test-balance`. **Policy drift:** payouts are "manual only" per §10 (admins pay from `/dashboard/payouts`). `payouts-cron`, the automated RazorpayX payout job, was unscheduled in migration 032 and removed in 2026-09; `escrow-release` only queues payouts for the admin. |
...
- **RazorpayX** — `RAZORPAYX_KEY_ID`, `RAZORPAYX_KEY_SECRET`, `RAZORPAYX_ACCOUNT_NUMBER` (only `register-payout-method` / `razorpayx-test-balance` remain).
- **Cron** — `CRON_SECRET`: shared secret pg_cron sends as `x-cron-secret` to `iap-expiry-sweep` (which chains `subscription-lapse-sweep`). Was also `payouts-cron`'s, now retired.
