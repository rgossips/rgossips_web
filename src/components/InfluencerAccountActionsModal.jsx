...
// Influencer sibling of BrandAccountActionsModal. `variant="deactivate"` is the
// soft, self-restorable pause (signing back in reactivates). `variant="delete"`
// is the 30-day pending_deletion purge — only admin can restore in the grace
// window (see whatsapp-otp-verifier, which blocks sign-in for both roles).

const REASONS_DEACTIVATE = [
  { key: "break", value: "Taking a break from social media" },
  { key: "otherPlatform", value: "Found another platform" },
  { key: "privacy", value: "Privacy concerns" },
  { key: "notEnoughCollabs", value: "Not getting enough collaboration opportunities" },
...
export default function InfluencerAccountActionsModal({ variant, open, onClose }) {
  const t = useTranslations("InfluencerAccountActionsModal");
  ...
  // Delete needs an extra typed-confirmation gate ("DELETE").
  const canSubmit =
    !!reason &&
    confirmed &&
    !submitting &&
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Boot every signed-in device. For delete, the verifier also blocks a
      // fresh OTP sign-in while pending_deletion.
      await supabase
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400 font-semibold">
                {isDelete ? t("subtitle.delete") : t("subtitle.deactivate")}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                  ...
                  className={`w-full flex items-center justify-between gap-3 p-3.5 rounded-xl border text-left transition-all cursor-pointer ${
                    reason === r.value
                      ? isDelete
                        ...
                        : "border-[#E60076] bg-pink-50/60"
                      : "border-gray-200 bg-white hover:border-gray-300"
                  }`}
                >
                  <span
                    className={`text-[12px] font-bold ${
                      reason === r.value ? (isDelete ? "text-red-700" : "text-[#E60076]") : "text-gray-700"
                    }`}
                  >
                  ...
                  >
                    {reason === r.value && (
                      <span className={`w-2 h-2 rounded-full ${isDelete ? "bg-red-500" : "bg-[#E60076]"}`} />
                    )}
                  </span>
                </button>
              ))}
          ...
          <label className="flex items-start gap-3 p-4 bg-white border border-gray-200 rounded-xl cursor-pointer">
            ...
            <span className="text-[12px] font-bold text-gray-700 leading-snug">
              {isDelete ? t("confirmCheckbox.delete") : t("confirmCheckbox.deactivate")}
            </span>
          </label>
