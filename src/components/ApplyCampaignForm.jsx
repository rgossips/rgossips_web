...
import AddReelFlow from "./AddReelFlow";
...
export function ApplyCampaignForm({ onClose, campaignData }) {
  ...
  const handleNewReelAdded = (newReel) => {
    // 1. Add the new reel to your local list (if you have one)
    // 2. Automatically select it
    setFormData((prev) => ({
      ...prev,
      selectedReels: [...prev.selectedReels, newReel.id],
    }));
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={`fixed inset-0 z-[60] lg:inset-auto lg:top-1/2 lg:left-1/2 lg:transform lg:-translate-x-1/2 lg:-translate-y-1/2 ${isAddingReel ? "lg:w-[90%] lg:max-w-[30vw] h-full lg:max-h-[95vh]" : "lg:w-[90%] lg:max-w-2xl lg:max-h-[90vh]"} lg:rounded-xl bg-white flex flex-col overflow-y-auto scrollbar-hide lg:mb-16 lg:mb-0 lg:shadow-2xl`}
      >
        {isAddingReel ? (
          <div className="h-full bg-white flex-1">
            <AddReelFlow
              onBack={() => setIsAddingReel(false)}
              onSuccess={handleNewReelAdded}
            />
          </div>
        ) : (
          <>
            ...
            <div className="p-6 flex items-center gap-4 border-b border-slate-50 sticky top-0 bg-white z-10">
              <button
                onClick={onClose}
                className="w-10 h-10 rounded-full cursor-pointer bg-[#FFEBF5] flex items-center justify-center text-[#E60076]"
              >
                <ArrowLeft size={20} />
              </button>
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 bg-[#B38B59] rounded-xl flex items-center justify-center text-white text-2xl font-bold">
                  {campaignData?.initials ||
                    campaignData?.title.slice(0, 2).toUpperCase() ||
                    "S"}
                </div>
                <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center cursor-pointer gap-2 p-4 rounded-xl border-2 transition-all ${
              ...
              <section className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-xs cursor-pointer font-bold text-[#E60076] hover:underline"
                  >
                    + Add New
                  </button>
                </div>
                  ...
                  {[
                    ...
                    >
                      <Checkbox
                        ...
                        className="w-5 h-5 cursor-pointer rounded-lg border-slate-300 data-[state=checked]:bg-[#E60076] data-[state=checked]:border-[#E60076]"
                      />
              ...
              <section className="space-y-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                <label className="flex items-start gap-3 cursor-pointer">
                  <Checkbox
                    ...
                    className="w-5 h-5 rounded-lg cursor-pointer border-slate-300 data-[state=checked]:bg-[#E60076] data-[state=checked]:border-[#E60076] mt-0.5 flex-shrink-0"
                  />
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      I understand all of guidelines comply to original and
                      maintain quality of promotion
                    </p>
                  </div>
                </label>
              </section>
