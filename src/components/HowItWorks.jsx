...
function TimelineItem({ step, index }) {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-center w-full mb-8 md:mb-12">
        ...
        {isLeft ? (
          ...
          >
            <div className="flex items-center justify-end gap-4 mb-3">
              ...
              <div className="p-3 bg-indigo-50 rounded-2xl">{step.icon}</div>
            </div>
      ...
      {/* CENTER: Number (Always centered on the line) */}
      <div
        className={`relative flex items-center justify-center z-10 ${!isLeft ? "pr-10" : "pl-10"}`}
      >
        <div className="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold shadow-lg shrink-0">
          {index + 1}
        </div>
      </div>
      ...
      <div className="w-full md:w-1/2 flex justify-start items-center">
        {!isLeft ? (
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: index * 0.1 }}
            className="w-[90%] bg-white p-8 rounded-2xl border border-slate-100 shadow-sm text-left ml-8 mt-6 md:mt-0"
          >
          ...
          /* Mobile view needs the content here even if isLeft, 
             so we handle mobile stacking separately */
          <div className="md:hidden block w-full px-4">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm text-left mt-6"
            >
