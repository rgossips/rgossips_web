...
export default function Pricing() {
  ...
  const planRows = (() => {
    ...
    const rows = [
      ...
      { label: "AI content suite", v: ["25 /mo", "150 /mo", "Unlimited"] },
    ...
    <section id="pricing" data-screen-label="Pricing" style={{ background: "linear-gradient(180deg, #F8F7FC, #FFFFFF)", scrollMarginTop: "80px" }}>
      ...
      <div className="prc-wrap" style={{ maxWidth: "1280px", margin: "0 auto", padding: "88px 40px 0" }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
          <h2 className="prc-title" style={{ fontSize: "42px", fontWeight: 800, letterSpacing: "-0.025em", margin: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "18px", flexWrap: "wrap" }}>
            ...
            >
              ...
              >
                Annual <span style={{ fontSize: "10px", fontWeight: 800, background: "#ECFDF5", color: "#10B981", borderRadius: "999px", padding: "2px 8px" }}>−20%</span>
              </button>
            </div>
          </div>
        </div>
        <div
          ...
          style={{
            marginTop: "48px",
            background: "linear-gradient(#FFFFFF, #FFFFFF) padding-box, linear-gradient(135deg, rgba(244,114,182,0.5), rgba(168,85,247,0.25) 45%, rgba(124,58,237,0.5)) border-box",
            border: "1.5px solid transparent",
            borderRadius: "28px",
            overflow: "hidden",
            boxShadow: "0 14px 40px rgba(124,58,237,0.1)",
          }}
        >
          <div className="prc-planshead" style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr 1fr" }}>
            <div className="prc-intro" style={{ padding: "28px 30px", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
              ...
              <p style={{ fontSize: "13px", color: "#6B7280", lineHeight: 1.6, margin: "8px 0 0" }}>
                Every plan, feature by feature. Click a plan to see exactly what you'd get — and what you'd miss.
              </p>
            </div>
            {plans.map((pl, i) => (
              <div
                key={i}
                onClick={pl.select}
                className="prc-plancol"
                style={{ position: "relative", padding: "24px 22px", cursor: "pointer", borderLeft: "1px solid #F1F0F5", background: pl.headBg, transition: "background 0.25s ease" }}
              >
                {/* {pl.popular && (
                  // <span
                  //   style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <span
                    style={{
                      ...
                      height: "16px",
                      borderRadius: "50%",
                      border: `2px solid ${pl.radioBorder}`,
                      background: pl.radioBg,
                      boxShadow: pl.radioShadow,
                ...
                <div style={{ marginTop: "12px" }}>
                  <span className="prc-price" style={{ fontSize: "34px", fontWeight: 800, letterSpacing: "-0.02em" }}>
                    {pl.price}
                  </span>
                  <span style={{ fontSize: "12px", color: "#9CA3AF" }}>{pl.per}</span>
                </div>
                ...
                <Link
                  ...
                  style={{
                    ...
                    background: pl.ctaBg,
                    color: pl.ctaFg,
                    border: `1.5px solid ${pl.ctaBorder}`,
                    boxShadow: pl.ctaShadow,
                    transition: "all 0.25s ease",
                  }}
                >
                  {pl.cta}
          ...
          {planRows.map((row, i) => (
            <div key={i}>
              {row.isGroup && (
                <div
                  className="prc-grouphead"
                  style={{ padding: "16px 30px 9px", borderTop: "1px solid #F1F0F5", fontSize: "10.5px", fontWeight: 800, letterSpacing: "0.14em", color: "#A855F7", background: "#FCFBFE" }}
                >
                  <span className="prc-grouplabel">{row.label}</span>
                </div>
              )}
              {row.isFeature && (
                <div
                  className="pricing-feat-row prc-featrow"
                  style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr 1fr", alignItems: "stretch", borderTop: "1px solid #F7F6FA", transition: "background 0.2s ease" }}
                >
                  <span className="prc-featlabel" style={{ padding: "12px 30px", fontSize: "13.5px", fontWeight: 700, color: "#4B5563", display: "flex", alignItems: "center" }}>
                    {row.label}
                  </span>
                  {row.cells.map((cell, ci) => (
                    <span
                      key={ci}
                      className="prc-featcell"
                      style={{
                        padding: "12px 18px",
                        borderLeft: "1px solid #F1F0F5",
                        background: cell.bg,
                        display: "flex",
                        alignItems: "center",
        ...
        <div
          className="prc-banner"
          style={{
            position: "relative",
            marginTop: "22px",
            background: "linear-gradient(115deg, #7C3AED, #A855F7 55%, #EC4899)",
            borderRadius: "24px",
            padding: "30px 36px",
            color: "#FFFFFF",
            overflow: "hidden",
            ...
            gap: "20px",
            flexWrap: "wrap",
            boxShadow: "0 18px 44px rgba(124,58,237,0.3)",
          }}
        >
          <div
            style={{
          ...
          <Link
            href="/login"
            className="pricing-brand-cta prc-cta"
            style={{
              position: "relative",
              background: "#FFFFFF",
              color: "#7C3AED",
              ...
              borderRadius: "999px",
              fontSize: "14px",
              boxShadow: "0 10px 24px rgba(30,27,46,0.2)",
      ...
      <style>{`
        @media (max-width: 767px) {
          ...
          /* Full desktop-style comparison table on mobile: the card scrolls
             horizontally, every row keeps its 4-column grid at fixed widths,
             and the first (label) column pins with position: sticky. */
          .prc-table { overflow-x: auto !important; overflow-y: hidden !important; -webkit-overflow-scrolling: touch; scrollbar-width: none; -ms-overflow-style: none; }
          .prc-table::-webkit-scrollbar { display: none; }
          .prc-planshead, .prc-featrow { grid-template-columns: 140px 140px 140px 140px !important; min-width: 536px !important; }
          .prc-grouphead { min-width: 536px !important; padding: 14px 14px 8px !important; }
          .prc-grouplabel { position: sticky; left: 14px; display: inline-block; }
          .prc-intro { position: sticky !important; left: 0 !important; z-index: 3 !important; background: #FFFFFF !important; padding: 14px 12px 12px !important; box-shadow: 2px 0 6px rgba(17,24,39,0.05); }
          .prc-intro p { display: none !important; }
          .prc-plancol { padding: 14px 12px !important; }
          .prc-price { font-size: 22px !important; }
          .prc-featlabel { position: sticky !important; left: 0 !important; z-index: 2 !important; background: #FFFFFF !important; padding: 12px 12px !important; font-size: 12.5px !important; box-shadow: 2px 0 6px rgba(17,24,39,0.05); }
          .prc-featcell { padding: 10px 8px !important; font-size: 12px !important; }
