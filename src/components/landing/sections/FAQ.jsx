...
export default function FAQ() {
  ...
  return (
    <section id="faq" data-screen-label="FAQ" style={{ maxWidth: "920px", margin: "0 auto", padding: "88px 40px 0", scrollMarginTop: "80px" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: "13px", fontWeight: 800, letterSpacing: "0.12em", color: "#A855F7" }}>FAQ</div>
        <h2 style={{ fontSize: "42px", fontWeight: 800, letterSpacing: "-0.025em", margin: "14px 0 0" }}>Questions, answered.</h2>
      </div>
        ...
        {faqs.map((q, i) => {
          ...
          return (
            <div key={i} style={{ border: `1.5px solid ${border}`, borderRadius: "18px", background: bg, overflow: "hidden", boxShadow: "0 4px 14px rgba(124,58,237,0.06)", transition: "all 0.25s ease" }}>
              <button onClick={() => setOpenFaq(open ? -1 : i)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "16px", padding: "19px 24px", background: "transparent", border: "none", cursor: "pointer", textAlign: "left", fontFamily: "'Manrope', sans-serif" }}>
                <span style={{ fontSize: "15.5px", fontWeight: 800, color: "#111827", letterSpacing: "-0.01em" }}>{q.q}</span>
              ...
              <div style={{ maxHeight: maxH, overflow: "hidden", transition: "max-height 0.35s ease" }}>
                <p style={{ padding: "0 56px 20px 24px", margin: 0, fontSize: "14px", color: "#6B7280", lineHeight: 1.75 }}>{q.a}</p>
              </div>
            </div>
          );
        })}
      </div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "22px", marginTop: "30px", flexWrap: "wrap", fontSize: "13.5px", fontWeight: 700 }}>
